...
public class SinhVien extends javax.swing.JFrame {
    ...
    public SinhVien(String username, String password) {
        ...
        initComponents();
        lblMSV.setText(currentUser);
        conFirmXoa.setVisible(false);
    ...
    private void hienThiTenSinhVien() {
        try {
            Connection conn = DatabaseConnection.connect();
            String sql = "SELECT HoTen FROM SV WHERE MaSV = ?";
            ...
            if (rs.next()) {
                ...
                lblStudentName.setText("Không tìm thấy tên sinh viên.");
            }
            rs.close();
            pst.close();
        } catch (SQLException e) {
            e.printStackTrace();
            lblStudentName.setText("Đã xảy ra lỗi khi truy vấn tên sinh viên.");
        }
    }
    ...
    private void ShowAvatar(String maSV) {
        try (Connection connection = DatabaseConnection.connect()) {
            ...
            if (rs.next()) {
                String anh = rs.getString("Anh");
                ImageIcon icon = new ImageIcon(anh);
                Image image = icon.getImage().getScaledInstance(86, 86, Image.SCALE_SMOOTH); // Resize ảnh
                icon = new ImageIcon(image);
                Show.setIcon(icon);
            } else {
    ...
    private void initComponents() {
        ...
        conFirmXoa = new javax.swing.JPanel();
        ...
        jLabel16.setText("Bạn có chắc muốn xóa Phiếu mượn này?");
        ...
        Khong.setText("Không");
        Khong.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                KhongMouseClicked(evt);
            }
        });

        javax.swing.GroupLayout conFirmXoaLayout = new javax.swing.GroupLayout(conFirmXoa);
        conFirmXoa.setLayout(conFirmXoaLayout);
        conFirmXoaLayout.setHorizontalGroup(
            conFirmXoaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(conFirmXoaLayout.createSequentialGroup()
                .addGap(25, 25, 25)
                .addGroup(conFirmXoaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(conFirmXoaLayout.createSequentialGroup()
                        .addComponent(Co)
        ...
        conFirmXoaLayout.setVerticalGroup(
            conFirmXoaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(conFirmXoaLayout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jLabel16)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addGroup(conFirmXoaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(Khong)
                    .addComponent(Co))
            ...
            .addGroup(phieuMuonLayout.createSequentialGroup()
                .addGroup(phieuMuonLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    ...
                    .addGroup(phieuMuonLayout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                        .addComponent(conFirmXoa, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addGroup(phieuMuonLayout.createSequentialGroup()
                        .addGroup(phieuMuonLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(phieuMuonLayout.createSequentialGroup()
                                .addGap(67, 67, 67)
                        ...
                        .addGroup(phieuMuonLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, phieuMuonLayout.createSequentialGroup()
                                ...
                                .addGap(180, 180, 180))
                            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, phieuMuonLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
        ...
        lblMSV.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        lblMSV.setForeground(new java.awt.Color(0, 51, 255));
        lblMSV.setText("jLabel18");

        lblStudentName.setFont(new java.awt.Font("Segoe UI", 1, 14)); // NOI18N
        lblStudentName.setForeground(new java.awt.Color(0, 51, 255));
        lblStudentName.setText("jLabel18");
    ...
    private void TaoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TaoMouseClicked
        ...
        int count = 0;
        try (Connection conn = DatabaseConnection.connect()) {
            String query = "SELECT count(*) FROM PM WHERE NgayTra IS NULL";
            PreparedStatement stmt = conn.prepareStatement(query);
        ...
        if (count >= 5) {
            JOptionPane.showMessageDialog(this, "Chỉ được phép mượn tối đa 5 phiếu mượn.\n Hãy trả Sách để mượn thêm!");
        }
    ...
    private void XoaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_XoaMouseClicked
        ...
        try {
            Connection conn = DatabaseConnection.connect();
            // Kiểm tra ngày hiện tại so với NgayMuon
            String sql = "SELECT NgayMuon FROM PM WHERE MaPM = ?"; // Giả sử MaPM được chọn từ giao diện
            PreparedStatement pst = conn.prepareStatement(sql);

            // Thay thế tham số MaPM bằng giá trị thực tế, ví dụ từ trường nhập liệu
            int selectedRow = tablePM.getSelectedRow();            
            if (selectedRow == -1) {
                JOptionPane.showMessageDialog(this, "Vui lòng chọn một phiếu mượn để xóa.");
                return;
            }

            // Lấy giá trị MaPM từ hàng được chọn
            int selectedMaPM = (int) tablePM.getValueAt(selectedRow, 0); // Giả sử cột 0 chứa MaPM

            pst.setInt(1, selectedMaPM);
            
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                java.sql.Date ngayMuon = rs.getDate("NgayMuon");
                if (ngayMuon != null && today.after(ngayMuon)) {
                    canDelete = false;
                }
            }
            rs.close();
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi khi kiểm tra ngày mượn.");
            return;
        }

        // Xử lý theo kết quả kiểm tra
        if (!canDelete) {
            JOptionPane.showMessageDialog(this, "Không thể xóa Phiếu mượn đã mượn thành công.");
        } else {
            conFirmXoa.setVisible(true); // Hiển thị hộp thoại xác nhận xóa
        }
    ...
    private void SuaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_SuaMouseClicked
        ...
        // Xử lý theo kết quả kiểm tra
        if (!update) {
            JOptionPane.showMessageDialog(this, "Không thể sửa Phiếu mượn đã mượn thành công.");
        } else {
            updatePhieuMuon();
        }
    ...
    private void KhongMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_KhongMouseClicked
        conFirmXoa.setVisible(false);
    }//GEN-LAST:event_KhongMouseClicked
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                SinhVien sv = new SinhVien("SV", "password");
                sv.setVisible(true);
    ...
    private javax.swing.JPanel conFirmXoa;
